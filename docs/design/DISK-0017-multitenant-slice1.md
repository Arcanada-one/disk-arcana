...
| 3 (merged #66) | Scoped `exchange_state` MetaDb path, baselines + conflicts tenant bind | Separate DB files per tenant |
| 4 (this PR) | `TenantMetaRouter`, `DISK_TENANT_DB_DIR`, per-tenant `meta.sqlite` | Auth Arcana JWT (DISK-0016) |
