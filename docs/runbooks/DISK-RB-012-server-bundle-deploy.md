...
The bundle contains exactly the binary, systemd unit, deployment helper,
broker, provisioner, sudoers policy, commit file, and `manifest.sha256`. The
...
1. From the approved tailnet root channel, create
   `/var/lib/disk-arcana/bootstrap/<deployment-id>` as `root:root 0700`.
...
4. Run the bundle's reviewed provisioner as root:

   ```text
   bash /var/lib/disk-arcana/bootstrap/<deployment-id>/bundle/provision-deploy-broker.sh \
     --bundle /var/lib/disk-arcana/bootstrap/<deployment-id>/bundle \
     --authorization /var/lib/disk-arcana/bootstrap/<deployment-id>/authorization
   ```

5. Require `state=COMMITTED`. Confirm the bootstrap directory is absent, the
...
6. Read back `sudo -ln -U <runner-user>`. The only task-owned NOPASSWD grant may
...
`target=stage`. For production, dispatch it once with `target=prod`; do not
