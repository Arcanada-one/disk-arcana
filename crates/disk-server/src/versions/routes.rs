...
use disk_core::meta_db::FileVersionUpsert;
...
async fn list_versions_inner(
    ...
    if query.path.is_empty() || query.path.contains("..") {
    ...
    let current = db
        .get_file_scoped(tenant_key, &query.vault_id, &query.path)
        .await
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "database error"))?;

    let rows = db
        .list_file_versions(tenant_key, &query.vault_id, &query.path, query.limit)
        .await
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "database error"))?;
...
async fn restore_version_inner(
    ...
    let version = db
        .get_file_version(tenant_key, &body.vault_id, &body.path, body.version_id)
        .await
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "database error"))?
    ...
    let target = path_guard::validate(Path::new(&body.path), &state.sync_root)
    ...
    if target.exists() {
        if let Ok(current_bytes) = std::fs::read(&target) {
            if let Ok(Some(current)) = db
                .get_file_scoped(tenant_key, &body.vault_id, &body.path)
                .await
            {
                let _ = state
                    .version_blobs
