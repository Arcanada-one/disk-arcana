//! Session tenant validation (DISK-0017 slice 2).
...
fn upload_req(
    ...
    if let Some(t) = tenant {
        req.metadata_mut()
            .insert("x-disk-tenant", t.parse().unwrap());
    }
    req.metadata_mut()
...
#[tokio::test]
async fn wrong_tenant_header_rejected() {
    let root = tempdir().unwrap();
    let meta_db = MetaDb::open(&root.path().join("meta.db")).await.unwrap();
    let (port, cert_pem) = spawn_server(root.path().to_path_buf(), meta_db).await;
    let ch = connect(port, &cert_pem).await;

    let token = register_and_auth(&ch, "bound-node", "tenant-a").await;
    let mut sync = SyncServiceClient::new(ch);

    let err = sync
        .delta_upload(upload_req(
            "x.bin",
            b"payload",
            &token,
            Some("tenant-b"),
...
#[tokio::test]
async fn missing_header_inherits_bound_tenant() {
    let root = tempdir().unwrap();
    let meta_db = MetaDb::open(&root.path().join("meta.db")).await.unwrap();
