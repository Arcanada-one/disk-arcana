...
#[tokio::test]
async fn migration_014_file_versions_table_exists() {
    let dir = tempdir().expect("tempdir");
    let db = MetaDb::open(&dir.path().join("file-versions-schema.sqlite"))
        .await
        .expect("open");
    ...
    assert!(
        tables.iter().any(|t| t == "file_versions"),
...
#[tokio::test]
async fn migration_023_user_org_context_table_exists() {
    let dir = tempdir().expect("tempdir");
    let db = MetaDb::open(&dir.path().join("org-ctx-schema.sqlite"))
        .await
        .expect("open");
    ...
    assert!(
        tables.iter().any(|t| t == "user_org_context"),
        "user_org_context table must exist after migration 023; got {tables:?}"
    );
}
